...
const handleConfirm = (date) => {
  hideDatePicker();
  const formattedDate = date.toISOString().split('T')[0]; // Format de la date en YYYY-MM-DD
  setSelected(formattedDate); // Mettre à jour selected avec la date au format requis
...
const handleChange = (value, fieldName) => {//function to handle change
  ...
  if (fieldName === "date") {//if field name is date then format the value
      const dateObject = new Date(value); //create a date object
  formattedValue = dateObject.toISOString().split('T')[0];//format the date object to YYYY-MM-DD
 ...
 console.log("f230categories :categories", value);
  ...
  const handleSubmit = async (e) => {
    ...
    setFormData({
      date: selected,
      categories: categories, // Utilisez la valeur sélectionnée
      label: label,
      amount: amount,
    });
